#!/usr/bin/env python3
"""
A thin App Store Connect API client, enough for what JUMPjet needs.
...
APP_BUNDLE_ID = "com.mdeller.jumpjet"

# JUMPjet is a single iOS target: no extensions, no watch app, no macOS or
# visionOS leg. That also means no App Groups, which is the other manual step
# the API cannot do, so the app record is the only one that applies here.
BUNDLE_IDS = [
    ("com.mdeller.jumpjet", "JUMPjet", "IOS"),
]
...
if __name__ == "__main__":
    ...
    if command == "bundle-ids":
        ...
        apps = app_records()
        target = [a for a in apps if a["bundleId"] == APP_BUNDLE_ID]
        print(f"team {TEAM_ID}, {len(apps)} app records visible")
        if target:
            print(f"  JUMPjet app record EXISTS: {target[0]}")
        else:
            print("  JUMPjet app record: NOT FOUND")
